...
const BASE_PROMPT = `あなたはZERO-PAINセルフケアアプリの専属AIカウンセラーです。20年以上の経験を持つベテランカイロプラクターとして、深い専門知識と豊富な臨床経験を持っています。
...
async function buildUserContext(deviceId: string): Promise<UserContextResult> {
  if (!deviceId) {
    return { contextText: "", latestPostureImageUrl: null, latestPostureDate: null, latestPostureDaysAgo: null, latestMealImageUrl: null, latestMealInfo: null };
  }
...
科学的に最適な推奨値: ${rec.recommendedCalories}kcal / タンパク質${rec.recommendedProteinG}g / 炭水化物${rec.recommendedCarbsG}g / 脂質${rec.recommendedFatG}g`
  ...
  return { contextText, latestPostureImageUrl, latestPostureDate, latestPostureDaysAgo, latestMealImageUrl, latestMealInfo };
}

export async function POST(req: NextRequest) {
  try {
    const { messages, deviceId, consultMeal } = await req.json();
    ...
    const { contextText, latestPostureImageUrl, latestPostureDate, latestPostureDaysAgo, latestMealImageUrl, latestMealInfo } =
    ...
    const shouldAttachMeal =
      isFirstMessage && consultMeal === true && latestMealImageUrl;

    // 姿勢写真が「3日以内に撮影された最新のもの」のみ自動添付
    // 古い写真（4日以上前）は毎回「拝見しました」と言うのは不自然なので添付しない
    ...
    if (shouldAttachMeal && latestMealImageUrl && latestMealInfo) {
