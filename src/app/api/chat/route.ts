...
科学的に最適な推奨値: ${rec.recommendedCalories}kcal / タンパク質${rec.recommendedProteinG}g / 炭水化物${rec.recommendedCarbsG}g / 脂質${rec.recommendedFatG}g`
  ...
  // 姿勢チェックが1件も無いユーザーには、会話の流れの中で1度だけ自然に勧めてもらう。
  // 2026-09-13: 登録直後にカメラを強制起動する導線をやめたため、ここが姿勢チェックの主な入口になる。
  // 押し付けると元の「撮影が関門」問題に戻るので、条件を細かく指定している。
  const postureInviteBlock =
    postures.length === 0
      ? `\n\n【姿勢チェックのお誘いについて】\nこのユーザーはまだ一度も姿勢チェック（全身写真の記録）をしていません。次を必ず守ってください。\n- まず相談内容にしっかり答える。お誘いはそのあと、最後に一言だけ添える。\n- 毎回は勧めない。会話が一区切りついたタイミングで1度だけにする。\n- 一度断られたら、それ以降は勧めない。\n- 必須だと思わせる言い方や、急かす言い方はしない。\n- 「ホームの Step 2」からできること、全身を2枚撮るだけで2分ほどで終わることを伝える。\n- 口調はあなたのキャラクター設定に合わせる。例:「一度、姿勢を撮って残しておくと、次に相談してくれた時にもっと具体的に見られるんじゃがな。ホームの Step 2 からできるぞ。今すぐでなくてええ」`
      : "";

  const contextText =
    parts.length === 0
      ? `\n\n${relationshipBlock}${postureInviteBlock}`
