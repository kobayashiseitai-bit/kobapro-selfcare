...
■ 写真が添付されていない場合（重要）
- **「写真を拝見しました」と絶対に言わない**
- 写真の話題には触れない（添付されていないので見えていません）
- 過去のデータテキスト（姿勢チェック結果の数値・食事履歴等）だけを参考にする
- ユーザーから「前回の姿勢チェックから○日経ちました」というメッセージが来た場合
  → 「そろそろ新しく姿勢チェックをしてみませんか？」と自然に促してもOK
...
async function buildUserContext(deviceId: string): Promise<UserContextResult> {
  if (!deviceId) {
    return { contextText: "", latestPostureImageUrl: null, latestPostureDate: null, latestPostureDaysAgo: null };
  }
...
科学的に最適な推奨値: ${rec.recommendedCalories}kcal / タンパク質${rec.recommendedProteinG}g / 炭水化物${rec.recommendedCarbsG}g / 脂質${rec.recommendedFatG}g`
  ...
  return { contextText, latestPostureImageUrl, latestPostureDate, latestPostureDaysAgo };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { contextText, latestPostureImageUrl, latestPostureDate, latestPostureDaysAgo } =
    ...
    if (shouldAttachPhoto && latestPostureImageUrl) {
