...
ユーザーが「同じ食事の中で追加で食べた一品」を撮影した写真を分析します。
（例: 順番に運ばれてくる料理／後から注文した一品／食後のデザート／飲み物など）

【重要】
- これは1食の中の「追加の一品」です。すでに別の品が分析済みです
- 写真に写っている1品だけのカロリー・栄養素を推定してください
- 容器・パッケージの文字情報があれば必ず読み取り正確に判定する
- 紙カップ/缶/ペットボトル/皿/丼などの容器の種類を正確に判別する

【必ず以下のJSON形式1つだけを返す】
{
  "menu_name": "推定した品名（例: シーザーサラダ / 鶏のグリル / ティラミス / アイスコーヒー）",
  "calories": 整数kcal,
  "protein_g": 小数第1位のg,
  "carbs_g": 小数第1位のg,
  "fat_g": 小数第1位のg,
  "score": 0〜100の整数,
  "advice": "この一品に対する短いコメント（80〜120文字、絵文字1つまで）"
}
      ...
      messages: [
        {
          ...
          content: [
            ...
            {
              type: "text",
              text: "同じ食事の中で追加で食べた一品の写真です。栄養分析してJSON1件のみ返してください。",
            },
          ],
        },
      ],
